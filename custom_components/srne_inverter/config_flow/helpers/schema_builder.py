...
class ConfigFlowSchemaBuilder:
    ...
    def build_schema(
        ...
        self._config_data.get("registers", {})
        page_registers = self._page_manager.get_page_registers(page_id, current_values)

        schema_dict = {}

        for reg_key, reg_data in page_registers:
            # Only show registers that have been successfully read from device
            if not current_values or reg_key not in current_values:
                _LOGGER.debug(
                    "Register %s not in coordinator data - skipping (not read from device)",
                    reg_key,
                )
                continue
            ...
            # Apply data type conversion and scaling using existing helper
            if default_value is not None and isinstance(default_value, (int, float)):
                default_value = process_register_value(
                    raw_value=int(default_value),
                    data_type=reg_data.get("data_type", "uint16"),
                    offset=reg_data.get("offset", 0),
                    scale=reg_data.get("scaling", 1.0),
                )
                _LOGGER.debug(
                    "Processed register value for %s: raw=%s, processed=%s",
                    reg_key,
                    current_values[reg_key],
                    default_value,
                )
            ...
            if selector_obj is None:
                _LOGGER.debug("Could not create selector for register %s", reg_key)
                continue

            # CRITICAL FIX: Convert default_value to string for select fields
            # Select options use string keys (e.g., "230") but coordinator returns
            # scaled values (e.g., 23.0 for 230V with 0.1 scaling).
            # Must un-scale and convert to match option keys.
            if default_value is not None:
                # Check if this is a select field (has "values" or "options")
                if reg_data.get("values") or reg_data.get("config_flow", {}).get(
                    "options"
                ):
                    # For select fields, options are defined as RAW register values
                    # But coordinator has SCALED values
                    # Need to un-scale before matching to options
                    scaling = reg_data.get("scaling", 1)
                    if scaling != 1 and isinstance(default_value, (int, float)):
                        # Un-scale: multiply by scaling to get raw value
                        # e.g., 23.0 * 10 = 230 (for 0.1 scaling)
                        raw_value = default_value / scaling
                        # Round to avoid floating point issues
                        raw_value = round(raw_value)
                        default_value = str(int(raw_value))
                        _LOGGER.debug(
                            "Un-scaled and converted default for select field %s: scaled=%s, raw=%s",
                            reg_key,
                            default_value,
                            raw_value,
                        )
                    else:
                        # No scaling or already correct type
                        default_value = str(
                            int(default_value)
            ...
            if default_value is not None:
                schema_dict[vol.Optional(reg_key, default=default_value)] = selector_obj
            else:
    ...
    def parse_user_input(
        ...
        for key, value in user_input.items():
            if key in registers:
                parsed[key] = SelectorFactory.parse_user_input(registers[key], value)
            else:
                parsed[key] = value
