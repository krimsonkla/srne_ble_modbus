...
This module provides pre-configured profiles for common use cases:
...
class PresetManager:
    ...
    def __init__(self, hass: HomeAssistant, coordinator: SRNEDataUpdateCoordinator) -> None:
        ...
        self._custom_presets_file = Path(hass.config.path(f"{DOMAIN}_custom_presets.json"))
    ...
    async def apply_preset(
        ...
        for setting_name, value in settings.items():
            try:
                ...
                # Write to inverter
                success = await self._coordinator.async_write_register(register, encoded_value)
    ...
    async def _validate_settings(self, settings: dict[str, Any]) -> list[str]:
        ...
        if "discharge_stop_soc" in settings and "switch_to_ac_soc" in settings:
            if settings["discharge_stop_soc"] >= settings["switch_to_ac_soc"]:
                errors.append(
                    "discharge_stop_soc must be less than switch_to_ac_soc"
                )
        ...
        if settings.get("timed_charge_enable") and "charge_start_time_1" not in settings:
            errors.append("timed_charge_enable requires charge_start_time_1")

        if settings.get("timed_discharge_enable") and "discharge_start_time_1" not in settings:
